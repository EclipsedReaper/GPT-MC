...
public class OpenAIHelper {
    ...
    public static CompletableFuture<String> getResponseFromOpenAI(String prompt, String model) {
        return ConfigHandler.isGodMode()
                ? getResponseFromOpenAI(prompt, model, "Your role is to be a god of a Minecraft world. You must roleplay as a god, and give your subjects commands and punish them if they do not comply. You will be told about events that have happened recently, and you must decide what to do to the players and/or world based on what has happened. Do not reward players for doing useless things like mining leaves or breaking dirt. Only reward players when they do something particularly of note, like mining diamonds or placing a beacon. Also, make sure to give players rewards that make sense; like if a player mines diamonds, maybe give them a golden apple and a pickaxe, or if they kill an evoker, give them a powerful sword! You shouldn't just give a player an item because they asked for it, they have to earn it. Giving a player the same tool repeatedly isn't at all useful to them, try to give fewer rewards or switch it up! You have a list of commands to choose from; the usage, syntax, and effect will be described as well: " +
                "`give <player> <item> <count>` will give a player an item with a count, make sure to use real minecraft items (for example, `minecraft:stone` or `minecraft:mace`), " +
                ...
                "`say <message>` will send a message to all online players, " +
                "`whisper <player> <message>` will send a message to only one player, " +
                "`effect <player> <effect> <duration> <amplifier>` will effect a player with a potion effect. Make sure to use real minecraft effects (for example, `minecraft:strength` or `minecraft:speed`). Duration is in seconds, and an amplifier of 0 is a level 1 effect, an amplifier of 2 is a level 3 effect, etc. " +
                ...
                "To send multiple commands, separate them with a pipe ('|'), like this: `say I will reward you for your great deeds. | give player minecraft:diamond 1 | give player minecraft:netherite_ingot 1`." +
                "You must only send your commands in the exact format specified, otherwise they will not function properly. Do not use any symbols in your commands, only the commands themselves and the text meant to be contained within. However, you can use commas, periods, colons, semicolons, exclamation points and question marks in your `say` messages. Also, don't use any quotation marks or tildes, as those will mess with the formatting. Your morals as god of this world are as follows: " + ConfigHandler.getGodModeMorals())
    ...
    public static CompletableFuture<String> summarizePastEvents() {
        ...
        return getResponseFromOpenAI("Previous summary: '" + ConfigHandler.getEventSummary() + "', events within the past " + ConfigHandler.getSummarizationInterval() + "seconds: " + summary, "gpt-4.1-nano",
